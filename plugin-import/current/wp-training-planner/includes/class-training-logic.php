...
class Training_Planner_Logic {
    ...
    public static function generate_month_sessions( $year, $month ) {
        ...
        for ( $day = 1; $day <= $num_days; $day++ ) {
            ...
            if ( $weekday == 3 ) {
                if ( $is_summer ) {
                    self::create_session( $date_str, '17:30:00', '19:30:00', __( 'Training for youth', 'training-planner' ) );
            ...
            elseif ( $weekday == 5 ) {
                if ( $is_summer ) {
                    self::create_session( $date_str, '17:30:00', '19:30:00', __( 'Training for youth', 'training-planner' ) );
                    self::create_session( $date_str, '19:30:00', '22:00:00', __( 'Training for adults', 'training-planner' ) );
                } else {
                    self::create_session( $date_str, '17:00:00', '19:00:00', __( 'Training for youth', 'training-planner' ) );
                    self::create_session( $date_str, '20:30:00', '22:15:00', __( 'Training for adults', 'training-planner' ) );
                }
            }
            // Saturday (6)
            elseif ( $weekday == 6 ) {
                if ( $is_summer ) {
                    self::create_session( $date_str, '10:00:00', '12:00:00', __( 'Open', 'training-planner' ) );
                } else {
                    self::create_session( $date_str, '10:00:00', '12:00:00', __( 'Training for youth', 'training-planner' ) );
                }
            }
        }
    ...
    private static function create_session( $date, $time, $end_time, $topic ) {
        ...
        $wpdb->insert(
            ...
            array(
                ...
                'location' => __( 'Sporthalle Gymnasium, Tettnang', 'training-planner' ),
            ),
            array( '%s', '%s', '%s', '%s', '%s' )
        );
    }
